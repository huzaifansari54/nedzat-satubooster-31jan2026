...
const WPP = require('./wa_wppconnect');
...
async function attachReactionsToRows(tenantId, accId, jid, rows, msgRefFn) {
  if (!rows?.length) return rows;
  const refs = rows.map(r => msgRefFn(r)).filter(Boolean);
  if (!refs.length) return rows;

  const qMarks = refs.map(() => '?').join(',');
  const list = await all(
    `SELECT msg_ref, reactions_json FROM msg_reactions
     WHERE tenant_id=? AND acc_id=? AND jid=? AND msg_ref IN (${qMarks})`,
    [tenantId, accId, jid, ...refs]
  );
  const map = new Map(list.map(x => [x.msg_ref, x.reactions_json]));
  for (const r of rows) {
    const mr = msgRefFn(r);
    r.msg_ref = mr;
    r.reactions_json = map.get(mr) || '';
  }
  return rows;
}
...
async function initDB() {
  ...
  // ✅ WhatsApp engine: baileys | wppconnect
  ...
  // Ensure is_active column exists for backwards compatibility
  await ensureCol('admin_notifications', 'is_active', 'INTEGER DEFAULT 1');
  ...
  await ensureCol('campaigns', 'is_active', 'INTEGER DEFAULT 1');
...
app.post('/api/broadcast/select_profiles', authGuard, requireFeature(FEATURES.BROADCAST), async (req, res) => {
  ...
  const jids = rows.map(r => r.jid).filter(Boolean);
  // res.json({ ok:true, count:jids.length, jids });
  const items = jids.map(jid => ({ jid, phone: jidToPhone(jid) }));
...
const wppClients = new Map(); // accId -> { client, tenantId }
  ...
  try {
    ...
    if (accKind === 'tg') {
      ...
      if (accData && Number(accData.waba_enabled) === 1) {
        ...
        const isWpp = rec?.engine === 'wppconnect' || rec?.isWpp || sock?.__isWpp;

        // wsState: сначала из rec.wsState, иначе из sock.ws.readyState
        let wsState = (typeof rec?.wsState === 'number')
          ? rec.wsState
          : (sock?.ws?.readyState);

        // для WPPConnect если wsState пустой — считаем OPEN
        if (isWpp && (wsState == null)) wsState = 1;

        // для WPPConnect если нет user.id — подставим, чтобы не падало в WA_NOT_READY
        if (isWpp && !sock?.user?.id) {
          const fallbackId = rec?.meJid || `wpp:${accId}`;
          sock.user = { ...(sock.user || {}), id: fallbackId };
        }

        if (!sock || !sock.user?.id || (!isWpp && typeof wsState === 'number' && wsState !== 1)) {
          console.warn('[AI][WA_NOT_READY] acc=%s jid=%s user=%s ws=%s',
            accId, jid, sock?.user?.id || '', String(wsState)
          );

          // ВАЖНО: offline ставим только для Baileys, WPPConnect не должен улетать в offline из-за ws
          if (!isWpp) {
            try { await setAccStatus(accId, 'offline', null); } catch (_) { }
          }
          return;
...
function wppToWaJid(wppFrom) {
  // wppFrom обычно вида "7701....@c.us"
  const s = String(wppFrom || '').trim();
  const digits = s.replace(/\D+/g, '');
  if (!digits) return '';
  return digits + '@s.whatsapp.net';
}

async function handleWppIncoming(accId, m) {
  if (!m) return;
  if (m.isGroupMsg) return;

  const acc = await get(`SELECT tenant_id FROM accounts WHERE id=?`, [accId]);
  const tenantId = Number(acc?.tenant_id || 1);

  const jid = normalizeDirectJid(wppToWaJid(m.from));
  if (!jid) return;
  ...
  // анти-дубль
  const waId = String(m.id || m._id || '').trim();
  const waKey = waId ? `wpp:${waId}` : '';

  if (waKey) {
    const dup = await get(
      `SELECT id FROM chats WHERE tenant_id=? AND acc_id=? AND jid=? AND wa_id=?`,
      [tenantId, accId, jid, waKey]
    );
    if (dup) return;
  }
  ...
  await run(
    `INSERT INTO chats(tenant_id,jid,date,ts,message,type,acc_id,wa_id)
     VALUES(?,?,?,?,?,?,?,?)`,
    [tenantId, jid, date, ts, txt, 'in', accId, waKey || null]
  );
...
async function startAccount(accId) {
  ...
  try {
    ...
    // --- WA ENGINE SWITCH (Baileys | WPPConnect) ---
    const waEngine = String(acc.wa_engine || 'baileys').toLowerCase();

    // --- WPPConnect блок (ИСПРАВЛЕНО) ---
    if (waEngine === 'wppconnect') {
      const folder =
        (acc.folder && String(acc.folder).trim())
          ? String(acc.folder).trim()
          : path.join('wpp', `acc-${accId}`);

      await fsp.mkdir(folder, { recursive: true });

      if (!acc.folder || String(acc.folder).trim() !== folder) {
        await run(`UPDATE accounts SET folder=?, updated_at=? WHERE id=?`, [folder, Date.now(), accId]);
        acc.folder = folder;
      }

      const tenantRoom = `tenant_${tenantId}`;

      console.log(`[WPP] Starting WPPConnect for acc ${accId}, this may take 30-60 seconds...`);

      const startTimeout = setTimeout(() => {
        console.warn(`[WPP] acc ${accId} taking long time to start, but continuing...`);
      }, 30000);

      let sockLike; // ✅ ИСПРАВЛЕНО: объявляем до try

      try {
        const result = await WPP.startSession({ // ✅ ИСПРАВЛЕНО: сохраняем в result
          accId,
          sessionName: `acc-${accId}`,
          sessionDir: path.resolve(folder),

          onQR: async (qrDataUrl) => {
            lastQRMap.set(accId, qrDataUrl);
            io.to(tenantRoom).emit('acc:qr', { id: accId, qr: qrDataUrl });

            const rec = sockets.get(accId);
            const alreadyOnline = rec?.sock?.ws?.readyState === 1;
          ...
          onOnline: async (me) => {
            ...
            io.to(tenantRoom).emit('acc:qr', { id: accId, qr: null });

            console.log(`[WPP] QR popup closed for acc ${accId}`); // ← ДОБАВЬ ЭТО

            // ✅ ИСПРАВЛЕНИЕ: обновляем meJid в sockets после получения номера
            const sockEntry = sockets.get(accId);
            if (sockEntry && me) {
              sockEntry.meJid = me;
              console.log(`[WPP] acc ${accId} updated meJid: ${me}`);
            }

            await setAccStatus(accId, 'online', me || null);
          },

          onOffline: async (reason) => {
            const r = String(reason || '').toLowerCase();

            try { await WPP.closeSession(accId); } catch (_) { }
            try { sockets.delete(accId); } catch (_) { }
            try { await setAccStatus(accId, 'offline', null); } catch (_) { }

            const isLogout = r.includes('unpaired') || r.includes('notlogged');
            if (isLogout) {
              reconnectAttempts.delete(accId);
              setTimeout(() => startAccount(accId).catch(console.error), 5000);
            }
          },

          onMessage: async (m) => {
            console.log(`[WPP] acc ${accId} message received from: ${m.from}`); // ← ДОБАВЬ
            try {
              const fromWpp = String(m.from || '').trim();
              const fromCanon = normalizeDirectJid(fromWpp.replace(/@c\.us$/i, '@s.whatsapp.net'));

              const cands = [
                m.from, m.chatId, m.sender?.id, m.sender?.chatId, m.sender?.jid, m.sender?.user, m.author
              ].filter(Boolean).map(v => String(v).trim()).filter(Boolean);

              // если прилетел @lid, попробуем найти альтернативный телефонный jid
              const best = cands.find(v => !/@lid$/i.test(v) && v !== 'status@broadcast') || fromWpp || cands[0] || '';

              let jid = normalizeDirectJid(best.replace(/@c\.us$/i, '@s.whatsapp.net'));

              // если раньше в базе был @lid, а теперь удалось получить телефонный jid — мигрируем, чтобы история не дробилась
              if (fromCanon && /@lid$/i.test(fromCanon) && jid && jid !== fromCanon && /@s\.whatsapp\.net$/i.test(jid)) {
                try {
                  await run(`UPDATE profiles SET jid=? WHERE tenant_id=? AND acc_id=? AND jid=?`, [jid, tenantId, accId, fromCanon]);
                  await run(`UPDATE chats    SET jid=? WHERE tenant_id=? AND acc_id=? AND jid=?`, [jid, tenantId, accId, fromCanon]);
                  await run(`UPDATE blocks   SET jid=? WHERE tenant_id=? AND jid=?`, [`${accId}:${jid}`, tenantId, `${accId}:${fromCanon}`]);
                  await run(`UPDATE satu_leads SET jid=? WHERE tenant_id=? AND acc_id=? AND jid=?`, [jid, tenantId, accId, fromCanon]);
                } catch (e) {
                  console.warn('[WPP JID_MIGRATE] warn:', e?.message || e);
                }
              }
              const ts = Math.floor(Date.now() / 1000);
              const wa_id = String(m.id || `wpp_${Date.now()}`);
              ...
              // Сохраняем контакт (НЕ затираем имя пустыми значениями)
              try {
                await run(
                  `INSERT INTO contacts (acc_id, jid, push_name, notify, updated_at)
                 VALUES (?, ?, ?, ?, ?)
                 ON CONFLICT(acc_id, jid) DO UPDATE SET
                   ...
                   updated_at = excluded.updated_at`,
                  [accId, jid, pushName || '', notify || '', ts]
                );
              } catch (err) {
                console.error(`[WPP] acc ${accId} save contact error:`, err);
              }

              // Сохраняем имя в profiles (чтобы в UI было имя, а не номер)
              try {
                const disp = String(pushName || notify || '').trim();
                if (disp) {
                  const prof = await getProfile(accId, jid);
                  const cur = String(prof?.name || '').trim();
                  const looksLikePhone = /^\+?\d{7,}$/.test(cur.replace(/\s+/g, ''));
                  const isBareJid = cur === jid.replace(/@.*/, '');
                  if (!cur || looksLikePhone || isBareJid || cur === 'WhatsApp User') {
                    await saveProfile(accId, jid, { name: disp.slice(0, 80) });
                  }
                }
              } catch (e) {
                // не падаем из-за профиля
                console.warn(`[WPP] acc ${accId} save profile name warn:`, e?.message || e);
              }
              // КОНЕЦ ДОБАВЛЕНИЯ

              // ✅ Определяем тип сообщения
              let messageType = 'text';
              let messageText = String(m.body || '');
              let mediaFile = null;
              let mediaKind = null;
              let mediaMime = null;
              let mediaName = null;
              ...
              if (m.type === 'chat' || m.type === 'text') {
                messageType = 'text';
              } else if (m.type === 'image' || m.mimetype?.includes('image')) {
                messageType = 'media';
                mediaKind = 'image';
                mediaMime = m.mimetype || 'image/jpeg';
                mediaName = m.filename || 'image.jpg';
                mediaSize = m.size || 0;
                messageText = m.caption || '';

                // Скачиваем медиа если есть downloadMedia
                if (typeof m.downloadMedia === 'function') {
                  try {
                    const media = await m.downloadMedia();
                    if (media) {
                      const base64Data = media.data || media;
                      const mediaDir = path.join(__dirname, 'media', String(accId));
                      await fsp.mkdir(mediaDir, { recursive: true });

                      const ext = mediaMime.includes('png') ? '.png' : '.jpg';
                      const filename = `${wa_id}${ext}`;
                    ...
                    console.error(`[WPP] acc ${accId} media download error:`, err);
                  }
                }
              } else if (m.type === 'video' || m.mimetype?.includes('video')) {
                messageType = 'media';
                mediaKind = 'video';
                mediaMime = m.mimetype || 'video/mp4';
                mediaName = m.filename || 'video.mp4';
                mediaSize = m.size || 0;
                messageText = m.caption || '';
                ...
                if (typeof m.downloadMedia === 'function') {
                  try {
                    ...
                    console.error(`[WPP] acc ${accId} video download error:`, err);
                  }
                }
              } else if (m.type === 'audio' || m.type === 'ptt' || m.mimetype?.includes('audio')) {
                messageType = 'media';
                mediaKind = m.type === 'ptt' ? 'voice' : 'audio';
                mediaMime = m.mimetype || 'audio/ogg';
                mediaName = m.filename || 'audio.ogg';
                mediaSize = m.size || 0;
                ...
                if (typeof m.downloadMedia === 'function') {
                  try {
                    ...
                    if (media) {
                      ...
                      const ext = mediaMime.includes('mp3') ? '.mp3' : '.ogg';
                      const filename = `${wa_id}${ext}`;
                      const filepath = path.join(mediaDir, filename);
                    ...
                    console.error(`[WPP] acc ${accId} audio download error:`, err);
                  }
                }
                ...
                if (typeof m.downloadMedia === 'function') {
                  try {
                    ...
                    console.error(`[WPP] acc ${accId} sticker download error:`, err);
                  }
                }
              } else if (m.type === 'document' || m.mimetype?.includes('application')) {
                messageType = 'media';
                mediaKind = 'document';
                mediaMime = m.mimetype || 'application/octet-stream';
                mediaName = m.filename || 'document';
                mediaSize = m.size || 0;
                messageText = m.caption || `[Документ: ${mediaName}]`;
                ...
                if (typeof m.downloadMedia === 'function') {
                  try {
                    ...
                    if (media) {
                      ...
                      const filename = `${wa_id}_${mediaName}`;
                      const filepath = path.join(mediaDir, filename);
                    ...
                    console.error(`[WPP] acc ${accId} document download error:`, err);
                  }
                }
              }
              ...
              await run(
                `INSERT INTO chats (tenant_id, acc_id, jid, message, type, ts, date, wa_id, status, media_file, media_kind, media_mime, media_name, media_size)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'received', ?, ?, ?, ?, ?)`,
                [tenantId, accId, jid, messageText, messageType, ts, date, wa_id, mediaFile, mediaKind, mediaMime, mediaName, mediaSize]
              );
              ...
              try {
                await pushLeadToCRM({
                  ...
                  jid,
                  phone: jid.replace(/@.*/, ''),
                  last_message: messageText || '[Медиа]',
                  wa_display_name: pushName || notify || ''
                });
              } catch (err) {
                console.error('[WPP][CRM] push error:', err);
              }

              // ✅ Real-time обновление интерфейса
              let displayPhone = jid.replace(/@.*/, '');
              // Пропускаем LID маппинг пока не создана таблица

              io.to(`tenant_${tenantId}`).emit('newchat', {
                acc_id: accId,
                jid,
                phone: displayPhone,
                text: messageText || (mediaKind ? `[${mediaKind}]` : ''),
                date: date,
                media_file: mediaFile || '',
                media_kind: mediaKind || ''
              });
              console.log(`[WPP] acc ${accId} newchat emitted for ${jid}`);

              // ✅ AI ответ только на текст и аудио (если есть транскрипция)
              const aiEnabled = Number(acc.ai_enabled) === 1;

              if (aiEnabled) {
                console.log(`[WPP] acc ${accId} AI enabled, scheduling reply`); // ← ДОБАВЬ
                if (messageType === 'text' && messageText) {
                  await scheduleAIReply(accId, jid, true);
                  console.log(`[WPP] acc ${accId} AI reply scheduled for ${jid}`); // ← ДОБАВЬ
                } else if (mediaKind === 'voice' && mediaFile) {
                  // Можно добавить транскрипцию голосового через Whisper
                  // await scheduleAIReply(accId, jid, true);
                }
              }
            } catch (e) {
              console.error('[WPP] Message error:', e);
            }
          }
        });

        sockLike = result.sockLike; // ✅ ИСПРАВЛЕНО: присваиваем после создания

      } finally {
        clearTimeout(startTimeout);
        console.log(`[WPP] acc ${accId} session creation completed`);
      }
    ...
    const folder =
      (acc.folder && String(acc.folder).trim())
        ? String(acc.folder).trim()
        : path.join('auth', `acc-${accId}`);

    await fsp.mkdir(folder, { recursive: true });

    // если в БД folder был пустой — зафиксируем, чтобы дальше всё было стабильно
    if (!acc.folder || String(acc.folder).trim() !== folder) {
      await run(`UPDATE accounts SET folder=?, updated_at=? WHERE id=?`, [folder, Date.now(), accId]);
      acc.folder = folder;
    }
      ...
      if (upd.connection === 'open') {
        ...
        try {
          ...
          if (tenantId) {
            ...
            console.log(`[WPP] QR popup closed for acc ${accId}`);
          }
        } catch (err) {
          console.error(`[WPP] Close QR error:`, err);
        }
        // КОНЕЦ ДОБАВЛЕНИЯ
    ...
    sock.ev.on('messages.upsert', async m => {
      try {
        ...
        // 2) FIX LID: remoteJid=@lid → пробуем заменить на senderPn, НО только если senderPn != myJid
        try {
          const remote = String(msg.key?.remoteJid || '');
          const senderPn = String(msg.key?.senderPn || '');
          if (remote.endsWith('@lid') && senderPn.endsWith('@s.whatsapp.net')) {
            if (!myJid || senderPn !== myJid) {
              jid = senderPn;
            }
          }
        } catch (_) { }

        // ⭐ СОХРАНЯЕМ СВЯЗЬ LID → PHONE для объединения чатов
        try {
          const originalRemote = String(msg.key?.remoteJid || '');
          const finalJid = jid;

          // Если было @lid, а теперь @s.whatsapp.net - сохраняем связь
          if (originalRemote.endsWith('@lid') && finalJid.endsWith('@s.whatsapp.net')) {
            await run(
              `INSERT OR REPLACE INTO lid_mapping (lid, phone_jid, phone_number) 
           VALUES (?, ?, ?)`,
              [
                originalRemote,
                finalJid,
                finalJid.split('@')[0]  // Извлекаем номер
              ]
            );
            console.log(`[LID→PHONE] Saved: ${originalRemote} → ${finalJid}`);
          }
        } catch (e) {
          console.warn('[LID→PHONE] Save failed:', e?.message);
        }
        ...
        // video
          ...
          try {
            if (openaiKey && node?.jpegThumbnail) {
              const openai = new OpenAI({ apiKey: openaiKey });
          ...
          if (isPdf) {
            try {
              ...
              if (receiptOcrEnabled) {
                ...
                if (parsedSum) {
                  await confirmReceipt(accId, jid, media_file, pdfText, parsedSum);
                  userText = `[Чек PDF]\nАвтоматически распознано: ${Number(parsedSum.amount).toFixed(2)
        ...
        // реакция / системные / интерактивное
          ...
          if (emoji) {
            ...
            if (targetWaId) {
              ...
              WHERE tenant_id=? AND acc_id=? AND jid=? AND wa_id=?
              ORDER BY id DESC LIMIT 1`,
                [tenantId, accId, jid, targetWaId]
              );
              if (row?.id) msgRef = `chat:${row.id}`;
            }
            if (!msgRef && targetWaId) msgRef = `wa:${targetWaId}`;
        ...
        io.to(`tenant_${tenantId}`).emit('newchat', {
          acc_id: accId,
          jid,                          // сырой jid (как было)
          phone: jidToPhone(jid),       // ← добавили для UI
          text: userText,
          date,
          media_file,
          media_kind
        ...
        try {
          ...
          if (subscription && subscription.push_subscription) {
            ...
            const payload = JSON.stringify({
              title: `Сообщение от ${fromName?.name || jidToPhone(jid)}`,
              ...
              messages: [
                {
                  ...
                  content: [
                    ...
                    `Поле phone всегда равно "${phoneE164}" (номер уже известен и подтверждён).`,
...
async function stopAccount(accId) {
  ...
  if (rec?.engine === 'wppconnect') {
    try { await WPP.stopSession(accId); } catch (_) { }
    sockets.delete(accId);
    reconnectAttempts.delete(accId);
    lastQRMap.delete(accId);
    await setAccStatus(accId, 'offline');
...
async function logoutAccount(accId) {
  ...
  if (rec?.engine === 'wppconnect') {
    try { await WPP.logoutSession(accId); } catch (_) { }
    sockets.delete(accId);
    reconnectAttempts.delete(accId);
    lastQRMap.delete(accId);

    const acc = await get(`SELECT folder FROM accounts WHERE id=?`, [accId]);
...
// MOVED TO AFTER DB INIT - DO NOT START BEFORE DB IS READY
// setInterval(() => {
...
app.post('/api/accounts', authGuard, async (req, res) => {
  try {
    ...
    // ✅ ОДИН РАЗ объявляем wa_engine
    const wa_engine = String(req.body?.wa_engine || 'baileys').trim().toLowerCase();
    if (kind === 'wa' && !['baileys', 'wppconnect'].includes(wa_engine)) {
      return res.status(400).json({ ok: false, error: 'wa_engine_invalid' });
    }
    ...
    // ✅ folder по движку
    if (kind === 'wa' && id) {
      const folder = (wa_engine === 'wppconnect')
        ? path.join('wpp', `acc-${id}`)
        : path.join('auth', `acc-${id}`);
...
app.delete('/api/accounts/:id', authGuard, async (req, res) => {
  ...
  // ✅ Удаляем WPPConnect сессию
  if (waEngine === 'wppconnect') {
    const wppSessionPath = path.join(__dirname, 'wpp', `acc-${id}`);
    await rmrf(wppSessionPath);
    console.log(`[WPP] Deleted session folder for acc ${id}`);
  }
...
app.get('/api/reports/overall', authGuard, async (req, res) => {
  try {
    ...
    const since = Number(t?.created_at || 0) || nowSec();
    ...
    // пополнения SatuCoin по месяцам (все положительные транзакции)
    const topupRows = await all(`
      SELECT strftime('%Y-%m', datetime(created_at,'unixepoch')) AS ym,
    ...
    for (const r of (topupRows || [])) {
      if (!r?.ym) continue;
      topupByYm[r.ym] = Number(r.tokens_added || 0);
    }

    const months = ymList.map(ym => {
      const leads = leadsByYm[ym] || 0;
      const tokens = topupByYm[ym] || 0;
      return {
        ym,
        leads,
        tokens_added: tokens,
        sum_kzt: tokens // если у вас 1 SatuCoin = 1 ₸
      };
    });

    const totals = {
      leads_total: months.reduce((a, x) => a + (x.leads || 0), 0),
      tokens_added: months.reduce((a, x) => a + (x.tokens_added || 0), 0),
      sum_kzt: months.reduce((a, x) => a + (x.sum_kzt || 0), 0)
    };

    res.json({ ok: true, since, totals, months });
  } catch (e) {
...
app.get('/api/profiles', authGuard, async (req, res) => {
  ...
  if (accId) {
    ...
    try {
      ...
      if (kind === 'wa') {
        // фильтруем и нормализованный, и сырой (на всякий случай — чтобы скрыть старые записи)
        if (meNorm) { sql += ` AND jid <> ?`; params.push(meNorm); }
        if (meRaw && meRaw !== meNorm) { sql += ` AND jid <> ?`; params.push(meRaw); }
      }
    } catch (_) { }
  ...
  if (order === 'recent') {
    sql += ` ORDER BY COALESCE(slots_updated_at,0) DESC, acc_id, jid LIMIT ?`;
  } else {
    sql += ` ORDER BY acc_id, jid LIMIT ?`;
  }
  ...
  try {
    ...
    if (accKind === 'tg') {
      ...
      await run(
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?)`,
        [tenantId, jid, date, ts,
          finalText || (kind === 'image' ? '🖼 Изображение' : kind === 'video' ? '🎬 Видео' : kind === 'audio' ? '🎤 Голосовое' : '📄 Документ'),
    ...
    if (!kind && mf) {
      const guessed = inferKindFromPath(mf);
      kind = guessed || 'document';
    }
    ...
    // ✅ Проверяем это WPP или Baileys
    const isWpp = sockEntry?.engine === 'wppconnect' || sockEntry?.isWpp || sock?.__isWpp;

    console.log(`[WA][SEND][${isWpp ? 'WPP' : 'BAILEYS'}] tid=${tenantId} acc=${acc_id} jid=${jid} len=${text?.length || 0}`);

    const abs = mf ? path.resolve(__dirname, mf) : '';

    // ✅ WPP использует ДРУГОЙ формат!
    if (isWpp) {
      // WPPConnect format
      const toWpp = jid.replace('@s.whatsapp.net', '@c.us');

      if (mf && kind === 'image') {
        if (!fs.existsSync(abs)) return res.status(400).json({ ok: false, error: 'media not found' });
        const buf = fs.readFileSync(abs);
        const base64 = `data:image/jpeg;base64,${buf.toString('base64')}`;
        await sock.sendMessage(toWpp, { image: base64, caption: finalText || '' });

      } else if (mf && kind === 'video') {
        if (!fs.existsSync(abs)) return res.status(400).json({ ok: false, error: 'media not found' });
        const buf = fs.readFileSync(abs);
        const base64 = `data:video/mp4;base64,${buf.toString('base64')}`;
        await sock.sendMessage(toWpp, { video: base64, caption: finalText || '' });

      } else if (mf && kind === 'audio') {
        if (!fs.existsSync(abs)) return res.status(400).json({ ok: false, error: 'media not found' });
        const buf = fs.readFileSync(abs);
        const base64 = `data:audio/ogg;base64,${buf.toString('base64')}`;
        await sock.sendMessage(toWpp, { audio: base64, ptt: true });

      } else if (mf) {
        if (!fs.existsSync(abs)) return res.status(400).json({ ok: false, error: 'media not found' });
        const buf = fs.readFileSync(abs);
        const base64 = `data:application/octet-stream;base64,${buf.toString('base64')}`;
        await sock.sendMessage(toWpp, { document: base64 });

      } else if (finalText) {
        await sock.sendMessage(toWpp, { text: finalText });
      ...
      // BAILEYS format (как было)
      if (mf && kind === 'image') {
        if (!fs.existsSync(abs)) return res.status(400).json({ ok: false, error: 'media not found' });
        const buf = fs.readFileSync(abs);
        await sock.sendMessage(jid, { image: buf });
        if (finalText) { await sleep(200); await sendLongText(sock, jid, finalText, tenantId); }
        ...
        await sendLongText(sock, jid, finalText, tenantId);
...
app.post('/api/crm/send', async (req, res) => {
  ...
  try {
    ...
    if (lead_id) {
      ...
      // ✅ АВТОМАТИЧЕСКАЯ конвертация JID для WPP
      const sockEntry = sockets.get(prof.acc_id);
      const isWpp = sockEntry?.engine === 'wppconnect' || sockEntry?.isWpp;

      if (isWpp && prof.jid) {
        const originalJid = prof.jid;
        // Конвертируем любой формат в @c.us для WPP
        if (prof.jid.includes('@lid')) {
          prof.jid = prof.jid.replace('@lid', '@c.us');
        } else if (prof.jid.includes('@s.whatsapp.net')) {
          prof.jid = prof.jid.replace('@s.whatsapp.net', '@c.us');
        }
        if (originalJid !== prof.jid) {
          console.log(`[CRM→SEND][WPP_JID_FIX] ${originalJid} → ${prof.jid}`);
        }
      }
    ...
    const sock = sockEntry.sock;
    const isWpp = sockEntry?.engine === 'wppconnect' || sockEntry?.isWpp;
    console.log(`[CRM→SEND][ENGINE] ${isWpp ? 'WPP' : 'BAILEYS'} for acc ${prof.acc_id}`);
    ...
    const sendLocalMedia = async (abs, kind) => {
      ...
      if (isWpp) {
        ...
        if (kind === 'image') {
          const base64 = `data:image/jpeg;base64,${buf.toString('base64')}`;
          await sock.sendMessage(prof.jid, { image: base64, caption: textSafe || '' });
          return { savedFile: 'uploads/' + path.basename(abs), savedKind: 'image' };
        }
        if (kind === 'video') {
          const base64 = `data:video/mp4;base64,${buf.toString('base64')}`;
          await sock.sendMessage(prof.jid, { video: base64, caption: textSafe || '' });
          return { savedFile: 'uploads/' + path.basename(abs), savedKind: 'video' };
        }
        if (kind === 'audio') {
          const base64 = `data:audio/ogg;base64,${buf.toString('base64')}`;
          await sock.sendMessage(prof.jid, { audio: base64, ptt: true });
          return { savedFile: 'uploads/' + path.basename(abs), savedKind: 'audio' };
        }
        // document
        const base64 = `data:application/octet-stream;base64,${buf.toString('base64')}`;
        await sock.sendMessage(prof.jid, { document: base64, fileName: path.basename(abs) });
...
app.get('/api/chats/history', authGuard, async (req, res) => {
  try {
    ...
    let sql = `SELECT id, ts, type, message, media_file, media_kind,
    ...
    await attachReactionsToRows(req.user.tenant_id, acc_id, jidNorm, rows, (r) => `chat:${r.id}`);
