...
// const { Server } = require('socket.io'); (Replaced by modular setup)
...
// --- Socket.IO Modular Bridge ---
const { setupSocketIO } = require('./src/sockets');
const emitters = require('./src/sockets/emitters');
...
const io = setupSocketIO(server); // Modular setup includes middleware and handlers
...
app.post('/api/admin/users/topup', authGuard, adminOnly, async (req, res) => {
  try {
    ...
    try {
      ...
      try { emitters.emitAdminNotification(userId, { id: notifId }); } catch (_) { }
  ...
  try {
    ...
    try {
      ...
      for (const u of (users || [])) {
        ...
        try { emitters.emitAdminNotification(u.id, { id: notifId }); } catch (_) { }
      }
    } catch (_) { }
...
async function setAccStatus(id, status, me_jid = null) {
  ...
  emitters.emitAccountUpdate(tid, { id, status, me_jid: normMe || undefined });
}

async function broadcastAccList(tid) { emitters.emitAccountList(tid, await listAccountsByTenant(tid)); }
...
async function sendAIReplyNow(accId, jid) {
  ...
  try {
    ...
    try { emitters.emitTypingStatus(tenantId, { acc_id: accId, jid, on: true }); } catch (_) { }
    ...
    try { emitters.emitTypingStatus(tenantId, { acc_id: accId, jid, on: false }); } catch (_) { }
    ...
    sock.ev.on('connection.update', async upd => {
      if (upd.qr) {
        ...
        if (!registered) {
          ...
          emitters.emitAccountQR(tenantId, { id: accId, qr: upd.qr });
          await setAccStatus(accId, 'qr');
        } else {
      ...
      try {
        ...
        // реакция / системные / интерактивное
          ...
          if (emoji && targetWaId) {
            ...
            emitters.emitReactionUpdate(tenantId, {
              acc_id: accId,
              jid,
              msg_ref: msgRef,
              reactions_json: next
...
async function processFirstMsgJobs() {
  ...
  for (const j of jobs) {
    try {
      ...
      emitters.emitNewChat(j.tenant_id, {
        acc_id: j.acc_id,
        jid: j.jid,
...
app.get('/', optAuth, async (req, res) => {
  if (!req.user) return res.sendFile(path.join(__dirname, 'public', 'login.html'));
  ...
  try {
    ...
    // 6.5) Канал-специфика (IG/WABA): отправляем без Baileys-сокета
    {
      ...
      if (accKind === 'ig' || jidStr.startsWith('ig:')) {
        ...
        try {
          const date = new Date().toISOString().slice(0, 10);
          emitters.emitNewChat(prof.tenant_id, {
            acc_id: accIdNum,
            jid: `ig:${threadId}`,
            phone: threadId,
            type: 'out',
...
// -------------------------------------------------
// Socket.IO is now handled in src/sockets/index.js
// -------------------------------------------------
...
async function handleWABAIncoming(message, metadata) {
  try {
    ...
    if (message.type === 'reaction') {
      ...
      try {
        emitters.emitReactionUpdate(tenantId, { acc_id: accId, msg_ref: ref });
      } catch (_) { }
      return;
    }
